...
/* Reuse local_compact from quickselect_mpi.c */
                              ...
                              long long k_global, MPI_Comm comm)
{
    ...
    while (1) {
        ...
        /* ===== Global median-of-medians pivot ===== */

        int num_groups_local = (n_local + 4) / 5;
        elem_t *local_meds = (elem_t *)malloc(num_groups_local * sizeof(elem_t));

        for (int g = 0; g < num_groups_local; g++) {
            int start = g * 5;
            int len = (start + 5 <= n_local) ? 5 : (n_local - start);
            if (len <= 0) {
                local_meds[g] = INT_MAX;
                continue;
            }
            elem_t tmp[5];
            for (int i = 0; i < len; i++) tmp[i] = a[start + i];
            ...
            local_meds[g] = tmp[len / 2];
        }

        int *recvcounts = NULL;
        int *displs     = NULL;
        elem_t *all_meds = NULL;
        if (rank == 0) {
            recvcounts = (int *)malloc(size * sizeof(int));
            displs     = (int *)malloc(size * sizeof(int));
        }

        MPI_Gather(&num_groups_local, 1, MPI_INT,
                   recvcounts, 1, MPI_INT,
                   0, comm);
        ...
        if (rank == 0) {
            for (int i = 0; i < size; i++) {
                displs[i] = total_meds;
                total_meds += recvcounts[i];
            }
            all_meds = (elem_t *)malloc(total_meds * sizeof(elem_t));
        }

        MPI_Gatherv(local_meds, num_groups_local, MPI_INT,
                    all_meds, recvcounts, displs, MPI_INT,
                    0, comm);
        free(local_meds);

        elem_t pivot = 0;
        if (rank == 0) {
            pivot = mom_pivot(all_meds, total_meds);
            free(all_meds);
            free(recvcounts);
            free(displs);
        }
...
int main(int argc, char **argv) {
    ...
    elem_t res = parallel_momselect_mpi(&local_a, &local_n, k, MPI_COMM_WORLD);
    double t1 = elapsed_seconds();

    if (rank == 0) {
        printf("[MPI Median-of-Medians] n=%d k=%d procs=%d\n", n, k, size);
        printf("Result: %d\n", res);
