...
/* Parallel partition using OpenMP.
   Writes < pivot, == pivot, > pivot into tmp, then back into a. */
...
/* Randomized parallel Quickselect using OpenMP */
elem_t parallel_quickselect_omp(elem_t *a, int n, int k, int num_threads) {
    ...
    while (1) {
        ...
        int pivot_index = left + rand() % active_n;
...
int main(int argc, char **argv) {
    ...
    elem_t result = parallel_quickselect_omp(a, n, k, num_threads);
    double t1 = elapsed_seconds();

    printf("[OMP Quickselect] n=%d k=%d threads=%d\n", n, k, num_threads);
    printf("Result: %d\n", result);
